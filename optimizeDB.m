...
function [d] = euclidean(img1, img2, type)
     ...
     if type == 'diff'
        d = mean(sqrt( (img1(:,:,1) - img2(:,:,1))^2 + (img1(:,:,2) - img2(:,:,2))^2 + (img1(:,:,3) - img2(:,:,3))^2 ));
     elseif type == 'dist'
          d1 = mean( sqrt( (img1(:,:,1))^2 + (img1(:,:,2))^2 + (img1(:,:,3))^2 ) );
          d2 = mean( sqrt( (img2(:,:,1))^2 + (img2(:,:,2))^2 + (img2(:,:,3))^2 ) );
